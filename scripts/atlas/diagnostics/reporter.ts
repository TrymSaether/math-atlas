...
const SEV_STYLE: Record<
  Severity,
  { mark: string; paint: (s: string) => string; word: string }
> = {
...
export function reportDiagnostics(
  ...
  const visible = diags.filter(
    (d) => showSuggestions || d.severity !== "suggestion",
  );
...
export function printSummary(diags: Diagnostic[]): void {
  ...
  if (c.error)
    parts.push(
      red(`${MARK.error} ${c.error} error${c.error === 1 ? "" : "s"}`),
    );
  if (c.warning)
    parts.push(
      yellow(
        `${MARK.warning} ${c.warning} warning${c.warning === 1 ? "" : "s"}`,
      ),
    );
  if (c.suggestion)
    parts.push(
      cyan(
        `${MARK.suggestion} ${c.suggestion} suggestion${c.suggestion === 1 ? "" : "s"}`,
      ),
    );
