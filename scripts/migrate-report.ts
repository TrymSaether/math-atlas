...
const DEP_MAP: Record<string, AuthorableRelation> = {
  logical_dependency: "uses",
  definitional_dependency: "defined_in_terms_of",
  pedagogical_dependency: "motivated_by",
  assumption_dependency: "assumes",
  construction_dependency: "constructed_from",
  notation_dependency: "uses",
};
...
function convert(raw: any, t: Tally) {
  ...
  for (const e of g.edges ?? []) {
    ...
    if (!m) continue;
    if (FUZZY_RELS.has(e.type)) t.fuzzyRel++;
    else if (m.swap) t.swapRel++;
    else t.cleanRel++;
    if (m.swap) push(e.target, e.source, m.to);
    else push(e.source, e.target, m.to);
  }
