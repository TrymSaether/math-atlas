...
export default function InnerProductFigure({ nodeId }: FigureProps) {
  ...
  return (
    <figure className="m-0">
      <FigureFrame xDomain={[-0.6, 2.0]} yDomain={[-0.5, 1.5]} height={190} grid>
        {/* angle arc between u and v */}
        <Polyline points={arc(0.42, 0, theta)} color={DIA.alert} weight={STROKE.mark} />
        {/* projection of v onto u */}
        <Line.Segment point1={v} point2={proj} color={DIA.ok} weight={STROKE.mark} style="dashed" />
        <Vector tail={[0, 0]} tip={proj} color={DIA.ok} weight={STROKE.ref} />
        {/* the two vectors */}
        <Vector tail={[0, 0]} tip={U} color={DIA.accent} weight={STROKE.curve} />
        <Vector tail={[0, 0]} tip={v} color={DIA.codomain} weight={STROKE.curve} />
        <Point x={proj[0]} y={proj[1]} color={DIA.ok} svgCircleProps={{ r: DOT.base }} />
        {orthogonal && (
          <Polyline
            points={[
              [0.16, 0],
        ...
        <LaTeX at={[U[0] + 0.05, -0.12]} tex={String.raw`u`} color={DIA.accent} />
        <LaTeX at={[v[0] - 0.05, v[1] + 0.12]} tex={String.raw`v`} color={DIA.codomain} />
