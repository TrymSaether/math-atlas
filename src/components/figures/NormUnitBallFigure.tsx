...
const P_MAX = 12;
const P_STEP = 0.1;
const AXIS_X: [Vec2, Vec2] = [[-2.25, 0], [2.25, 0]];
const AXIS_Y: [Vec2, Vec2] = [[0, -1.95], [0, 1.95]];
              ...
              {mode.kind === "lp" && active && (
                <>
                  <input
                    ...
                    style={{ accentColor: "var(--fg-on-color)", background: "color-mix(in srgb, var(--fg-on-color) 28%, transparent)" }}
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 border-t pt-2 text-ui-meta" style={{ borderColor: "var(--border)", color: "var(--fg-2)" }}>
        <MathText text={`The solid outline is $\\|x\\|_{${subscript}}B_{${subscript}}$: the unit ball scaled until it reaches $x$.`} />
      </div>
    </aside>
  );
}
      ...
      <div className="mt-3 flex flex-col gap-3 2xl:flex-row">
        <div className="min-w-0 flex-[1.35]">
          <FigureFrame xDomain={[-2.4, 2.4]} yDomain={[-2.1, 2.1]} height={FIGURE_HEIGHT} axes={false} grid>
            ...
            {mode === "l2" ? (
              <Circle center={[0, 0]} radius={UNIT_RADIUS} color={DIA.codomain} fillOpacity={REGION_FILL_OPACITY} strokeOpacity={1} weight={STROKE.ref} />
            ) : (
              <Polygon points={ball} color={DIA.codomain} fillOpacity={REGION_FILL_OPACITY} strokeOpacity={1} weight={STROKE.ref} />
            )}
