import type { GraphData, GraphDomain, GraphEdge, GraphNode } from "@/maps/types";
import type { Artifact } from "@shared/maps/artifact";
import { computeSwimlaneLayout, type DomainBounds, type Position } from "./layout";
import { resolveDomainTones, type DomainTone } from "./colors";
import { computeGraphMetrics, type GraphMetrics } from "./graphMetrics";
...
export function buildAtlasMap(data: GraphData): AtlasMap {
...
export type BuildResult = { ok: true; map: AtlasMap; warnings: string[] } | { ok: false; error: string };

/**
 * Validate an edited source graph with the SAME strict schema the CLI build
 * uses, then re-derive the runtime AtlasMap. This is the in-browser mirror of
 ...
 */
export function buildAtlasMapFromSource(source: unknown): BuildResult {
  ...
  return { ok: true, map: buildAtlasMap(enrichArtifact(artifact)), warnings };
}
